...
public:
       ...
       void setSensorData(struct sensor_mag_s _sensor_mag, struct sensor_combined_s _sensor_combined);
...
private:
      ...
      Matrix<float,3,1> _mag_readings;

      Matrix<float,3,1> _mag_readings_hist;      

      Matrix<float,4,1> _gyro_readings;

      Matrix<float,4,1> _acc_readings;      
      ...
      void writeMagReadingsOnFile(const char *filename, hrt_abstime t); 
